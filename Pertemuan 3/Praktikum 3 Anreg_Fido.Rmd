...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\Lenovo\\Downloads\\Dataset Anreg.xlsx")
dt
```

```{r}
y <- dt$"ANGKA KEMATIAN BAYI (PER 1000 KELAHIRAN HIDUP)"
x <- dt$"UMUR HARAPAN HIDUP"
n <- length(dt)
```
...
$$E[\hat{Y}] = \hat{Y} = 282.35401 -3.61621 X$$
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 1670.16 dan JKG model bernilai 2.05

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 0.06421168. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
$$S_e = \sqrt{0.06421168} \approx 0.2534002$$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 282.354 dan -3.616215. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [278.3635 ,  286.3445] dan untuk parameter $β_1$ adalah [-3.67108 ,  -3.561349]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 246.1919. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 243.3304 hingga 249.0534.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 246.1919. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 243.2842 hingga 249.0995.