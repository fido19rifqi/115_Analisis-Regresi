...
data <- read_excel("C:\\Users\\hp\\Downloads\\NSC-2025.xlsx")
...
Saat ini terdapat 1 peubah Y dan 9 peubah X, misal ingin dilakukan eksplorasi dan analisis terkait peubah X5

```{r}
plot(data$X5, data$Y,
     main = "Kontribusi Sektor Industri Pengolahan teradap PDRB/PDB  (%) vs Indeks Pembangunan Manusia",
     xlab = "RLS",
     ylab = "IPM")
```
...
$$\hat{Y} = 75.42849  +0.01313829 X_1$$

dengan Y adalah persentase penduduk miskin dan X6 adalah persentase akses sanitasi layak.

Didapatkan nilai $\hat\beta_0 = 75.42849$ artinya nilai dugaan rataan indeks pembangunan manusia ketika rata-rata lama sekolah bernilai 0 persen adalah 50.353%, akan tetapi nilai 0 tidak ada dalam selang pengamatan rata-rata lama sekolah sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut
```{r}

```

Nilai $\hat\beta_0 = 0.01313829$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika Rata-rata lama sekolah berubah satu tahun adalah sebesar 2.875%

Koefisien determinasi yang dihasilkan adalah 0.8884 atau 88.84% artinya keragaman persentase penduduk miskin yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 88.84%, sisanya yaitu 11.16% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
